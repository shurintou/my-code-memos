function load() {
    ...
    if (!document.getElementById(document.title).checked) {
        newText.innerText = `but it's not. Because the checked property was auto completed. 
        Try change the autocomplete property to be 'off' .`
...
function select(event) {
    ...
    newText.innerText = `Autocomplete property has been changed, 
    because the new property would only be applied to new history, 
    so try click pageA(B) several times to make some histories :)`
    document.body.appendChild(newText)
}
